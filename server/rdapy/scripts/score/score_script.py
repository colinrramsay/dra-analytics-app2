...
def main():
    ...
    parser.add_argument('--precomputed', type=str, help='Path to precomputed data (optional)')
    ...
    parser.add_argument('--by-district', type=str, help='Path to by-district output file')
    parser.add_argument('--mode', type=str, default=default_values["mode"], 
                        help='Analysis mode (default: all)')
    parser.add_argument('--census', type=str, default=default_values["census"],
                        help='Census data to use (default: T_20_CENS)')
    parser.add_argument('--vap', type=str, default=default_values["vap"],
                        help='Voting age population data to use (default: V_20_VAP)')
    parser.add_argument('--cvap', type=str, default=default_values["cvap"],
                        help='Citizen voting age population data to use (default: V_20_CVAP)')
    parser.add_argument('--elections', type=str, default=default_values["elections"],
                        help='Election data to use (default: E_16-20_COMP)')
    parser.add_argument('--expand-composites', action='store_true',
                        help='Expand composite elections')
    ...
    required_args = ['state', 'plan_type', 'geojson', 'graph', 'plans', 'scores', 'by_district']
    missing_args = [arg for arg in required_args if not getattr(args, arg)]
    
    if missing_args:
        print("Error: Missing required arguments")
        print("Usage: score_script.py --state <xx> --plan-type <chamber> --geojson <path> --graph <path> "
        ...
        print("  --vap <vap>           Voting age population data to use (default: V_20_VAP)")
        print("  --cvap <cvap>         Citizen voting age population data to use (default: V_20_CVAP)")
    ...
    try:
        ...
        precomputed_flag = f"--precomputed {args.precomputed}" if args.precomputed else ""
