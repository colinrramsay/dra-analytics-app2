...
Using the repository at the command line requires Python 3.12, and pyenv is recommended for Python version management. To setup the repository for local command-line use:
...
If not already installed, use Homebrew to install pyenv and Python 3.12:
...
Create a virtual environment that uses Python 3.12 and reset Python outside the virtual environment to the normal setting.

```bash
pyenv shell 3.12
python3 -m venv ~/your/preferred/path/rdapy
...
```bash
source ~/your/preferred/path/rdapy/bin/activate
pip install -r requirements.txt
pip install --upgrade pip
```

From the project root, install the rdapy package in development mode to create a link in your `site-packages`.
```bash
pip install -e .
```
...
```bash
# First create an output folder, if one doesn't already exist
mkdir temp
